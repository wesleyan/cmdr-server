  ...
  module CmdrServer
    class Server
      def initialize
        ...
        @db_cmdr_server = CouchRest.database!("http://127.0.0.1:5984/cmdr_server")
      ...
      def set_up_browser
        EM.defer do
          ...
          browser = DNSSD.browse("_cmdr._tcp") do |client_reply|
      ...
      def save_doc req
        ...
        begin
          ...
          DaemonKit.logger.info "Error occured on request: #{req.inspect}"

          old = @db_rooms.get(req['doc']['_id'])
          DaemonKit.logger.info "Original doc: #{old.inspect}"
          old['_deleted'] = true
          @db_rooms.save_doc(old)
          req['doc'].delete('_rev')
          if tries < 5
            tries += 1
            retry
